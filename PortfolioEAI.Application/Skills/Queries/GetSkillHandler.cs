...
{
    public class GetSkillHandler : IRequestHandler<Interfaces.GetSkillQuery, Result<SkillDto?>>
    {
        // Repository for accessing skill data
        private readonly ISkillRepository _repository;

        //a Constructor to inject the skill repository
        public GetSkillHandler(ISkillRepository repository) => _repository = repository;

        // Handles the GetSkillByIdQuery request
        public async Task<Result<SkillDto?>> Handle(Interfaces.GetSkillQuery request, CancellationToken ct)
        {
            try
            {
                // Retrieve the skill by ID from the repository
                Skill? skill = await _repository.Get(s => s.Id == request.Id, ct);
